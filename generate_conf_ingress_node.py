...
class Neighbor:
    ...
    ospf_cost: Optional[int] = None
    ingress_for: Optional[str] = None  # interface locale dont cette session est l'ingress préféré (ex: "Loopback0")
...
class AutonomousSystem:
    ...
    def allocate_loopback(self) -> Union[ipaddress.IPv4Address, ipaddress.IPv6Address]:
        ...
        if not self.allow_as_in:
            ...
            # 1. On compte combien de routeurs doivent tenir dans le pool
            num_routers = len(self.routers)
            if num_routers == 0:
                return next(self.loopback_pool.hosts())

            # 2. On calcule combien de bits on doit ajouter au préfixe pour créer assez de réseaux
            # math.log2(num_routers) nous donne le nombre exact de bits
            # math.ceil arrondit à l'entier supérieur (ex: pour 3 routeurs, il faut 2 bits car 2^2=4)
            bits_needed = math.ceil(math.log2(num_routers))
            
            # 3. Nouveau préfixe (ex: si pool /24 et 4 routeurs, target = 26)
            target_prefix = self.loopback_pool.prefixlen + bits_needed
            
            # 4. Taille du saut (step) en nombre d'adresses
            step = 2**(32 - target_prefix)
                
            # 6. Attribution de la première IP utilisable du segment suivant
            ...
            for neigh in router.neighbors:
                if neigh.type == "intra-as":
                    ...
                    if neigh.interface not in router.interfaces:
                        ...
                        #print(router.name, neigh.router, neigh.interface, r_ip)
    ...
    if as_obj.ip_version == 4:
        ...
        if router.role != "CE":
            ...
            if not as_obj.allow_as_in:
                ...
                # 1. On calcule le nombre de bits à ajouter
                bits_needed = math.ceil(math.log2(len(as_obj.routers))) if len(as_obj.routers) > 1 else 0
                # 2. On l'ajoute au préfixe de la pool
                current_prefix = as_obj.loopback_pool.prefixlen + bits_needed
                lines += [f" ip address {router.loopback} {_mask(current_prefix)}"]  #lààààààààààààààààààà
            ...
            if router.role == "CE":
                ...
                if ingress_neighbors:
                    ...
                    for neigh in ingress_neighbors:
                        ...
                        primary_iface = neigh.ingress_for  # ex: "Loopback0"
                        pl_primary = f"PL_{primary_iface.replace('Loopback', 'L').replace('/', '_')}"
                        lines += [
                            f"route-map {rm_name} permit 10",
                            f" match ip address prefix-list {pl_primary}",
                            " set community 65636",
                            "!",
                        ]
